#ifndef		TEST_FOR_THE_LONGEST_RUN_OF_ONES_IN_A_BLOCK_CPP
#define		TEST_FOR_THE_LONGEST_RUN_OF_ONES_IN_A_BLOCK_CPP
...
namespace test{
    ...
    void TestForTheLongestRunOfOnesInABolock::run(std::vector<bool> &bits){
        P = test_for_the_longest_run_of_ones_in_a_block(bits, m);
    }

    const char* TestForTheLongestRunOfOnesInABolock::testName(int lang){
        char* names[1] = {"Test for the longest run of ones in a block"};
        return names[lang];
    }

    TestForTheLongestRunOfOnesInABolock::TestForTheLongestRunOfOnesInABolock(unsigned int _m) {m=_m;}
    TestForTheLongestRunOfOnesInABolock::~TestForTheLongestRunOfOnesInABolock() {}
