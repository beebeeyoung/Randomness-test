#ifndef		APPROXIMATE_ENTROPY_TEST_CPP
#define		APPROXIMATE_ENTROPY_TEST_CPP
...
namespace test{
    ...
    void ApproximateEntropyTest::run(std::vector<bool> &bits){
        P = approximate_entropy_test(bits, m);
    }

    const char* ApproximateEntropyTest::testName(int lang){
        char* names[1] = {"Approximate entropy test"};
        return names[lang];
    }

    ApproximateEntropyTest::ApproximateEntropyTest(unsigned int _m) {m=_m;}
