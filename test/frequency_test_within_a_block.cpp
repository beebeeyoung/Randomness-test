#ifndef		FREQUENCY_TEST_WITHIN_A_BLOCK_CPP
#define		FREQUENCY_TEST_WITHIN_A_BLOCK_CPP
...
namespace test{
    ...
    void FrequencyTestWithinABlock::run(std::vector<bool> &bits){
        P = frequency_test_within_a_block(bits, m);
    }

    const char* FrequencyTestWithinABlock::testName(int lang){
        char* names[1] = {"Frequency test within a block"};
        return names[lang];
    }

    FrequencyTestWithinABlock::FrequencyTestWithinABlock(int _m) {m = _m;}
    FrequencyTestWithinABlock::~FrequencyTestWithinABlock() {}
