#ifndef		SERIAL_TEST_CPP
#define		SERIAL_TEST_CPP
...
namespace test{
    ...
    void SerialTest::run(std::vector<bool> &bits){
        P = serial_test(bits, m);
    }

    const char* SerialTest::testName(int lang){
        char* names[1] = {"Serial test (1)"};
        return names[lang];
    }

    SerialTest::SerialTest(int _m) {m = _m;}
    SerialTest::~SerialTest() {}



    void SerialTest2::run(std::vector<bool> &bits){
        serial_test(bits, m);
        P = serial_test2(bits);
    }

    const char* SerialTest2::testName(int lang){
        char* names[1] = {"Serial test (2)"};
        return names[lang];
    }

    SerialTest2::SerialTest2(int _m) {m = _m;}
