#ifndef		POKER_TEST_CPP
#define		POKER_TEST_CPP
...
namespace test{
    ...
    void PokerTest::run(std::vector<bool> &bits){
        P = poker_test(bits, m);
    }

    const char* PokerTest::testName(int lang){
        char* names[1] = {"Poker test"};
        return names[lang];
    }

    PokerTest::PokerTest(int _m) {m = _m;}
    PokerTest::~PokerTest() {}
